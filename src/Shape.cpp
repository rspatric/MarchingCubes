...
void Shape::getNormals() {
   float faceNormals[eleBuf.size()];
   for (int i = 0; i < eleBuf.size(); i+=3) {
      float p1[3] = {posBuf[3*eleBuf[i]], posBuf[3*eleBuf[i]+1], posBuf[3*eleBuf[i]+2]};
      float p2[3] = {posBuf[3*eleBuf[i+1]], posBuf[3*eleBuf[i+1]+1], posBuf[3*eleBuf[i+1]+2]};
      float p3[3] = {posBuf[3*eleBuf[i+2]], posBuf[3*eleBuf[i+2]+1], posBuf[3*eleBuf[i+2]+2]};   
   ...
   int vertNum = -1;
   float tempNor[] = {0, 0, 0};
   //now get vertex normals
   for (int i = 0; i < posBuf.size(); i+=3) {
      vertNum++;
      for (int j = 0; j < eleBuf.size(); j++) {
         if(eleBuf[j] == vertNum) {
            count++;
            int face = j - (j % 3);
            tempNor[0] += faceNormals[face];
            tempNor[1] += faceNormals[face+1];
            tempNor[2] += faceNormals[face+2];
         }
      }
      tempNor[0] /= count;
      tempNor[1] /= count;
      ...
      norBuf[i] = tempNor[0];
      norBuf[i+1] = tempNor[1];
      norBuf[i+2] = tempNor[2];
      count = 0;
      tempNor[0] = 0;
      tempNor[1] = 0;
      tempNor[2] = 0;
